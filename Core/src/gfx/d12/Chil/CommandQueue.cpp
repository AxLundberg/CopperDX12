...
namespace CPR::GFX::D12
{
		...
		commandListType_{ commandListType }
	{
		...
		pDeviceInterface->CreateCommandQueue(&desc, IID_PPV_ARGS(&pCommandQueue_)) >> hrVerify;
		pDeviceInterface->CreateFence(currentFenceValue_, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&pFence_)) >> hrVerify;
	}
	AllocatorListPair CommandQueue::GetAllocatorListPair()
	{
		std::lock_guard lk{ mutex_ };

		AllocatorListPair pair;
		...
		if (!commandAllocatorQueue_.empty() && FenceHasReached(commandAllocatorQueue_.front().fenceValue)) {
			...
			pair.pCommandAllocator->Reset() >> hrVerify;
		}
		...
		if (!commandListPool_.empty())
		{
			...
			pair.pCommandList->Reset(pair.pCommandAllocator.Get(), nullptr) >> hrVerify;
		}
	...
	void CommandQueue::ExecuteCommandList(AllocatorListPair commandListPair)
	{
		std::lock_guard lk{ mutex_ };

		// close the command list
		commandListPair.pCommandList->Close() >> hrVerify;
		// execute the list
		{
	...
	uint64_t CommandQueue::ExecuteCommandListWithFence(AllocatorListPair commandListPair)
	{
		std::lock_guard lk{ mutex_ };
		...
		pCommandQueue_->Signal(pFence_.Get(), ++currentFenceValue_) >> hrVerify;
	...
	void CommandQueue::WaitForFenceValue(uint64_t fenceValue) const
	{
		if (!FenceHasReached(fenceValue)) {
			pFence_->SetEventOnCompletion(fenceValue, nullptr) >> hrVerify;
		}
	}
	...
	ComPtr<ID3D12CommandAllocator> CommandQueue::MakeCommandAllocator()
	{
		...
		pDevice_->GetD3D12DeviceInterface()->CreateCommandAllocator(
			commandListType_, IID_PPV_ARGS(&pCommandAllocator)) >> hrVerify;
	...
	ComPtr<ID3D12GraphicsCommandList2> CommandQueue::MakeCommandList(ComPtr<ID3D12CommandAllocator> pCommandAllocator)
	{
		...
		pDevice_->GetD3D12DeviceInterface()->CreateCommandList(0,
			commandListType_, pCommandAllocator.Get(), nullptr, IID_PPV_ARGS(&pCommandList)) >> hrVerify;
		return pCommandList;
	}
}
