...
namespace CPR::GFX::D11
{
	...
	void RendererD11::Present(int screenShot)
	{
		if(screenShot)
			SaveScreenCapture(screenShot);

		auto swapChain = deviceSwapchainAndContext->GetD3D11SwapChain();
		swapChain->Present(0, 0);
	}

	void RendererD11::SaveScreenCapture(int screenShot)
	{
		auto swapChain = deviceSwapchainAndContext->GetD3D11SwapChain();
		auto context = deviceSwapchainAndContext->GetD3D11DeviceContext();
		auto device = deviceSwapchainAndContext->GetD3D11Device();
		...
		std::string name = "screenshot" + std::to_string(screenShot) + ".png";
		stbi_write_png(name.c_str(), width, height, channels, convertedData, width * channels);
